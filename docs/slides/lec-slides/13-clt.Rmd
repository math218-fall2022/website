...
subtitle: "Introduction and Inference"
author: "Becky Tang"
date: "06.16.2021"
...
2. .vocab[Sample size / distribution:] 
...
The `p<dist>(x)` function in R lets us calculate the probability that a random variable that follows the specified distribution is less than $x$. 
...
The `q<dist>(p)` function in R lets us find which value of the random variable $X$ lies at the $p$-th percentile, when $X$ follows the specified distribution 
...
1. The distribution of the sample statistic is nearly normal

2. The distribution is centered at the (often unknown) population parameter

3. The variability of the distribution is inversely proportional to the square 
...
If necessary conditions are met, for a population with a well-defined mean $\mu$ and standard deviation $\sigma$,
these three properties hold for the distribution of sample average $\bar{X}$,
assuming certain conditions hold:
...
That is, the CLT tells us that $\bar{X}$ approximately has the distribution $N\left(\mu, \sigma/\sqrt{n}\right)$. 
