...
- We consider a subset of the `USArrests` data. Contains  statistics about arrests per 100,000 residents for $\color{blue}{\text{Murder}}$ in each of the 50 US states in 1973. Also given is the percent of the population living in urban areas $(\color{blue}{\text{UrbanPop}})$.
...
- Most common choice is *squared* Euclidean distance:
...
- Run $K$-means with $K = 4$, using features `compactness`, and `area`
...
```{r echo = T}
set.seed(1)
kmeans2 <- kmeans(scale(iris[,1:4]), 2, nstart = 20)
table(kmeans2$cluster, iris$Species)
```
...
- $k$-medoids: very similar to $k$-means clustering
  ...
  - $k$-means chooses centers of clusters as the average between the points in the cluster, thus the centroid usually is not one of the observed points
...
- Allows for greater interpretability of the center clusters than $k$-means 
...
- $k$-medoids minimizes the sum of pairwise dissimilarities, whereas $k$-means attempts minimizes sum of (squared) Euclidean distances
