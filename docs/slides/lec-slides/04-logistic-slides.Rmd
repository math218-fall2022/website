...
- What if we have $K > 2$ classes? Can extend the logistic regression model to  **multinomial** regression

- Here, we discuss the *softmax* encoding

--

- In softmax coding, we do not choose a baseline but rather, treat all classes symmetrically. Each class gets its own set of coefficients.
...
- Your homework will compare logistic encoding and softmax encoding for $K = 2$
