...
2. For every observation that fall in $R_{l}$, we output the same prediction: the mean of the training responses in $R_{l}$, $\hat{y}_{R_{l}} = \frac{1}{n_{l}} \sum_{i \in R_{l}} y_{i}$

--

- What does these $R_{l}$ look like?
...
- A better strategy is to grow a very large tree $T_{0}$, and then **prune** it back in order to obtain a smaller **subtree**
 ...
 - Idea: remove sections of $T_{0}$ that are non-critical
...
```{r}
gini <- function(x){sum(x*(1-x))}
ent <- function(x){
 temp <- x
 temp[which(temp==0)] <- 0.0000000000000001
 -sum(temp*log(temp))
}
p1 <- c(0.25, 0.25, 0.25,0.25)
p2 <- c(0.1, 0.1, 0.2, 0.6)
p3 <- c(0, 0, 0.05, 0.95)
g1 <- gini(p1); d1 <- round(ent(p1),3 )
g2 <- gini(p2); d2 <- round(ent(p2),3)
g3 <- gini(p3); d3 <- round(ent(p3), 3)

```

- If $K = 4$ and $(\hat{p}_{l1}, \hat{p}_{l2},\hat{p}_{l3},\hat{p}_{l4})$ = (`r p1`), then $G_{l} = `r g1`$ and $D_{l} = `r d1`$

- If $(\hat{p}_{l1}, \hat{p}_{l2},\hat{p}_{l3},\hat{p}_{l4})$ = (`r p2`), then $G_{l} = `r g2`$ and $D_{l} = `r d2`$
 
- If $(\hat{p}_{l1}, \hat{p}_{l2},\hat{p}_{l3},\hat{p}_{l4})$ = (`r p3`), then $G_{l} = `r g3`$ and $D_{l} = `r d3`$
...
```{r seeds, fig.width=6, fig.height=6}
...
- Advantages: 
 ...
 - May more closely mirror human decision-making than other approaches we've seen
...
- An **ensemble** method combines many simple "building block" models in order to obtain a single final model
...
- Recall that given a set of $n$ independent observations $Z_{1}, \ldots, Z_{n}$, each with variance $\sigma^2$, then $\text{Var}(\bar{Z}) = \sigma^2/n$

--

 - i.e. averaging a set of observations reduces variance. However, we typically don't have access to multiple training sets

--

## Bagging

- Instead, we can bootstrap! Take repeated samples from the single training set we have
...
- To apply bagging to regression trees: 
 ...
 - Trees are grown deep but not pruned
 
--

- For classification trees:

 - For each test observation, record the class $k$ predicted by each of the $B$ trees, and take a *majority vote*; the overall prediction is the most commonly occurring class among the B predictions
...
- Overall summary of important of each predictor using RSS or Gini index
  ...
  - Bagging classification trees: add up total amount that Gini indesx is decreased by splits over a given predictor, averaged over B
...
- **Random forests** provide improvement over bagged trees by providing small tweak that *decorrelates* the trees
...
## Boosting for regression treeds
