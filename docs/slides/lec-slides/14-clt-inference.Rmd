...
If the CLT tells us that $\bar{X}$ approximately has the distribution $N\left(\mu, \sigma/\sqrt{n}\right)$, then by properties of the normal distribution, we have
...
In practice, we never know the true value of $\sigma$, and so we estimate it from our data with $s$, the .vocab[sample standard deviation].

--

We cannot directly substitute $s$ for $\sigma$ and expect to have the same result 
as in the CLT. After all, $s$ is just an estimate. 

--

But we can make the following test statistic for testing
...
durham %>% 
  ...
  ggplot(aes(x = quality_library)) + 
    ...
    labs(x = "Satisfaction with public library system", y = "Count",
         title = "Most residents are generally satisifed with the library")
```
...
As we do not know the true $\sigma$ and we only have $s$, use the $t$-distribution.
...
.vocab[critical values] are cut-off values that define regions where the test statistic is unlikely to lie; related to the rejection region 
