...
knitr::include_graphics("img/06/box.png")
```
...
knitr::include_graphics("img/06/rain.jfif")
```
...
knitr::include_graphics("img/06/surgery.jpg")
```
...
```{r uc_berkeley, echo = F}
status <- c(rep("Admit",3738 + 1494), rep("Deny", 4704 +  2827))
knitr::kable(table(status))
```
...
.question[
$P(\text{admission}) = \dfrac{\text{# admitted}}{\text{# applied}} = \dfrac{5232}{5232 + 7531} \approx 0.41$
]
...
UC Berkeley admission figures for Fall of 1973, by gender:

.pull-left[
```{r uc_berkeley_men, echo = F}
men <- c(rep("Admit",3738), rep("Deny", 4704))
knitr::kable(table(men))
```
]

.pull-right[
```{r uc_berkeley_women, echo = F}
women <- c(rep("Admit",1494), rep("Deny", 2827))
knitr::kable(table(women))
```
]
...
.question[
- $\small{P(\text{man admitted}) =}$ ?
- $\small{P(\text{woman admitted})=}$ ? 
]
...
.question[
- $P(\text{man admitted}) = \dfrac{3738}{3738 + 4704} \approx 0.44$ 
- $P(\text{man admitted}) = \dfrac{1494}{1494 + 2827} \approx 0.35$ 
]
