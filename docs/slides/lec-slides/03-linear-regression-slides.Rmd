...
- A simple, widely used approach to supervised learning
...
- How strong is the relatinships between abalone age and the physical attributes?

- Which physical attributes are associated with age? What is the strength of the association?
...
- **Simple linear regression** assumes a linear model for a quantative response using a single predictor:

$$Y = \beta_{0} + \beta_{1} X + \epsilon,$$

  where $\beta_{0}, \beta_{1}$ are unknown **coefficients** or **parameters** and $\epsilon$ is the error term

- $\beta_{0}$ is commonly referred to as the **intercept**, and $\beta_{1}$ as the **slope**
...
- Once we have stimates for the coefficients, can predict future responses at $X = x$ using $$\hat{y} = \hat{\beta}_{0} + \hat{\beta}_{1}x$$
...
- To measure how close a resulting line is to the $n$ data points, we will minimize the *least squares* criterion

--

- Let $\hat{y}_{i} =  \hat{\beta}_{0} + \hat{\beta}_{1}x_{i}$ be prediction for $Y$ based on $i$-th value of $X$

- Then $i$-th **residual** is: $e_{i} = y_{i} - \hat{y}_{i}$

---

## Estimation of parameters by least squares

- Define **residual sum of squares** (RSS) as $$RSS = e_{1}^{2} + e_{2}^{2} + \ldots + e_{n}^{2}$$

- This is equivalent to $$RSS = (y_{1} - \hat{\beta}_{0}  - \hat{\beta}_{1}x_{1})^2 + (y_{2} - \hat{\beta}_{0}  - \hat{\beta}_{1}x_{2})^2 + \ldots (y_{n} - \hat{\beta}_{0}  - \hat{\beta}_{1}x_{n})^2$$ 
...
Least squares fit for regression of $\color{blue}{\text{age}}$ on $\color{blue}{\text{length}}$, with selected residuals.
...
- Note: the estimates of $\beta_{0}$ and $\beta_{1}$ will depend on the observed data!

---

### Assessing Accuracy of Coefficient Estimates

- Simulated example: true model is $Y = 1 + 3X + \epsilon$, where $\epsilon$ is error from mean-zero normal distribution

- We will generate $n = 100$ data points from this model and fit the least squares line to these data
...
- We can continue to simulate datasets of $n=100$ observations. Below are the least squares lines for each separate set of simulated data.
...
- *Standard error* (SE) of an estimator reflects how it varies under repeated sampling. For simple linear regression:

$$\text{SE}(\hat{\beta}_{0}) = \sigma^2 \left[ \frac{1}{n} + \frac{\bar{x}^2}{\sum_{i=1}^{n}(x_{i} - \bar{x})^2}\right], \qquad \text{SE}(\hat{\beta}_{1}) = \frac{\sigma^2}{\sum_{i=1}^{n}(x_{i} - \bar{x})^2}$$
...
- Typically $\sigma^2$ is not known, but can be estimated from the data. Estimate $\hat{\sigma}$ is residual standard error (RSE) given by:
...
- Standard errors can be used to compute **confidence intervals** for the coefficients! A 95% confidence interval (CI) is the range of values such that with 95% probability, the range will contain the true value of the parameter.
...
- **Null hypothesis** $H_{0}$: there is no relationship between $X$ and $Y$ versus
...
because if $H_{0}$ true, then the model would reduce to $Y = \beta_{0} + \epsilon$
...
- Can calculate **p-value**, which is the probability of observaing any value equal to $|t|$ or larger
...
- Compare $p$-value to a pre-determine rejection level $\alpha$ (often $\alpha = 0.05$). If $p$-value $< \alpha$, reject $H_{0}$
...
- **Residual standard error** (RSE) $$\text{RSE} = \sqrt{\frac{1}{n-2}\text{RSS}} = \sqrt{\frac{1}{n} \sum_{i=1}^{n} (y_{i} - \hat{y}_{i})^2}$$

  - Recall that $RSE$ is estimate of the standard deviation of $\epsilon$
...
- For the RSE from regressing age on length is `r round(RSE, 3)`, and the $R^2$ is `r round(R2, 3)`
...
- With $p$ predcitors, the model is $$Y = \beta_{0} + \beta_{1}X_{1} + \beta_{2}X_{2} + \ldots + \beta_{p}X_{p}$$
...
- Once again, estimate the coefficients as the values that minimize the sum of squared residuals $$RSS = \sum_{i=1}^{n}(y_{i} - \hat{y}_{i})^2$$
...
- Fitting coefficients for abalone data:
...
- Correlations for abalone data:

```{r}
round(cor(abalone[,c("age", "height", "length", "weight")]), 3)
```
...
$$H_{0}: \beta_{1} = \beta_{2} = \ldots = \beta_{p} = 0$$ versus
...
- For example, a simple linear regression model for $\color{blue}{\text{age}}$ would be
...
- Fitted model is: $$\hat{\color{blue}{\text{age}_{i}}} = `r coeffs[1]` + `r coeffs[2]` \color{blue}{\text{Male}_{i}}$$
...
$$x_{i1} = \begin{cases} 1 & \text{ if } \color{blue}{\text{sex}_{i}} = \text{Male} \\
...
Results in the regression model: 
\begin{align*}
y_{i} &= \beta_{0} + \beta_{1} x_{i1} + \beta_{2} x_{i2} + \epsilon_{i}\\
&= \begin{cases}  \beta_{0} + \beta_{1} & \text{ if } \color{blue}{\text{sex}_{i}} = \text{Male} \\
\beta_{0} + \beta_{2} & \text{ if } \color{blue}{\text{sex}_{i}} = \text{Female} \\
\beta_{0} & \text{ if } \color{blue}{\text{sex}_{i}} = \text{Infant} \end{cases}
\end{align*}
...
- There will always be one fewer dummy variables than levels
  ...
  - Level with no dummy variable is known as *baseline*
...
- This third predictor $\color{X_{1}X_{2}}$ is known as an **interaction** term or effect
...
- A 1 mm increase in the \color{blue}{\text{length}} of an abalone is associated with an increased age of `r coeffs[3]` + `r coeffs[4]` $\times \color{blue}{\text{height}}$ years
...
- In particular, interaction between a quantitative and a categorical variable has nice interpretation

--

- Consider again subset of abalone data where we remove the observations $\color{blue}{\text{sex}} = \text{Infant}$. Consider the effects of $\color{blue}{\text{height}}$ and  $\color{blue}{\text{sex}}$ on $\color{blue}{\text{age}}$.
...
- Can always fit a linear regression model for quantiative $Y$, but should assess whether the linear model is appropriate
...
- Additionally, check for potential problems that may influene model fit:
...
## 2. Assumption: uncorrelated errors
...
- Often assessed by identifiying *tracking* in the data. Plot residuals in the order they appear in the data
...
## 3. Assumption: constant variance
...
x <- rnorm(n, 0, 1)
beta_true <- c(1,-0.5) 
y <- cbind(1,x) %*% beta_true + rnorm(n, 0, 1) 
y[n] <- y[n] + 3.5
...
## 2. Problem: high leverage points
...
```{r fig.align="center", fig.height=5, fig.width=5}
x_lev <- -4
y_lev <- c(cbind(1, x_lev) %*% beta_true) + rnorm(1, 0,1) + 2.5
...
- Purple point is outlier from before. Purple line is fitted line omitting this point
...
- Can be hard to separate the individual effects of $\color{blue}{\text{weight}}$ and $\color{blue}{\text{length}}$ on $\color{blue}{\text{age}}$
