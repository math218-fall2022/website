...
- Although if using least square linear or polynomial regression, we have a nice shortcut!
...
Run $k$-fold CV with $K =$ `r K` run `r J` separate times, each with different random split of data:
...
x <- matrix(rnorm(n*p, 0, 1),nrow=n, ncol = p,byrow = T)
col_samps <- sample(1:p, p_cv)
ids0 <- which(y == 0)
ids1 <- which(y == 1)
x[ids0,] <- x[ids0, col_samps] + 1
x[ids1,] <- x[ids1, col_samps] -1
...
for(k in 1:K){
  ...
  mse_df[k] <- mean(y[val_set_ids]!=preds)
  
}
  
(wrong_cv <- mean(mse_df))
