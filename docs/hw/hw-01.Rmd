...
knitr::include_graphics("img/01/jovana-askrabic-XYIQXLH_v0o-unsplash.jpeg")
```
...
- Find the **hw-01-** repo on GitHub
- Clone this repo into your RStudio Docker container [https://vm-manage.oit.duke.edu/containers](https://vm-manage.oit.duke.edu/containers)
...
knitr::include_graphics("img/01/rstudio-anatomy.png")
```
...
knitr::include_graphics("img/01/ready-to-push.png")
```
...
knitr::include_graphics("img/01/load-packages-chunk.png")
```
...
knitr::include_graphics("img/01/view-data.png")
```
...
To do this, we will arrange the counts in descending order using the `arrange()` function. We pass in the name of the variable we want to sort by. Using the `desc()` function specifies that we want to arrange in descending order. Then to obtain the top five, we can use the `slice()` function.
The argument is the number of rows to select, which is `n = 5` for the top 5.
