...
#### Example
Let $\mathbf{a}= (1,2)$ and $\mathbf{b} = (3,5)$. Then $d(\mathbf{a}, \mathbf{b}) = \sqrt{(3-1)^2 + (5-2)^2} = \sqrt{13}$
...
b) Using the Euclidean distance metric, what is is our prediction with $K = 1$? Why?

c) Using the Euclidean distance metric, what is is our prediction with $K = 3$? Why?
...
e) Using the Manhattan distance metric, what is is our prediction with $K = 1$? Why?

f) Using the Manhattan distance metric, what is is our prediction with $K = 3$? Why?
...
d) Now, let's consider some new data with just one model for predictions: 
...
e) Now, suppose we measure a 7th point, where $y_{7} = 5$ and $\hat{y}_{7} = 15$. Clearly our prediction is way off! Calculate the MSE and MAE include this new observation, and comment on how they compare to the errors obtained on the original six data points.
...
f) Are there scenarios where we might prefer one error metric over the other?
