...
c) Now, suppose we had actually measured `x2 = 5x` (third column of table above). That is, `x2` is simply a *scaled* version of the original `x`. Calculate the correlation between  `x2` and `y`.
...
i) I have claimed that in the case of simple linear regression of Y onto X, the $R^2$ statistic is equal to the square of the correlation $r$ between X and Y. Prove that this is the case. For simplicity, you may assume that $\bar{x} = \bar{y} = 0$.


ii) Show that the least squares regression line always passes through the sample mean of the data. For simplicity, you may assume we are in the case of simple linear regression.
