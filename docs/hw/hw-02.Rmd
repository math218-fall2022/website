...
As it turns out, in simple linear regression, the $R^2$ value is exactly the square of the correlation coefficient $r$ (defined below) between the predictor $x$ and the response $y$! However, $R^2$ and $r$ have two very different meanings. We will explore both in terms in this homework.
...
Based on these plots, what are some cautions when it comes to interpreting Pearson's correlation coefficient (with respect to linearity, outliers). 
...
b) Looking at the plots above: for which value of the predictor $x$ are both intervals narrowest? Explain why this is the case.
