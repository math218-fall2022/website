...
b) The above plot displays 95% confidence intervals for predicted ozone for various `Temp` values. What do you think about the quality of our predictions for Ozone? Are you comfortable with all of these intervals? 
...
c) Now, suppose we actually measured `x2 = 5x`. That is, `x2` is simply a *scaled* version of the original `x`. Calculate the correlation between  $x_2$ and $y$.


```{r}
knitr::kable(df %>% mutate(x = 5*x) %>% rename("x2" = "x"))
```


d) Now, supposed we actually measured `x3 = x + 1`. That is, `x3` is just a shifted version of the original `x`. Calculate the correlation between  $x_3$ and $y$.


```{r}
knitr::kable(df %>% mutate(x = x+1) %>%  rename("x3" = "x"))
```
