...
Suppose that we take a data set, divide it into equally-sized training and test sets, and then try out two different classification procedures. First we use logistic regression and get an error rate of 25% on the training data and 30% on the test data. Next we use 1-nearest neighbors (i.e. $K = 1$) and get an average error rate (averaged over both test and training data sets) of 17%. Based on these results, which method should we prefer to use for classification of new observations? Why?
...
data.frame(X1 , Y) %>%
  group_by(X1, Y) %>% count %>% knitr::kable()
data.frame(X2 , Y) %>%
  group_by(X2, Y) %>% count %>% knitr::kable()
```

a) What are $f_{kj}(x_{j})$ for $k = 1, 2, 3$ and $j = 1,2$?


b) Assume that $\hat{\pi}_{1} = 0.3$,  $\hat{\pi}_{2} = 0.3$, and $\hat{\pi}_{3} = 0.4$.
...
c) Now let's say that our new observation is  $x^∗ = (\text{green},\text{ground})'$. For this new observation, what would $\text{Pr}(Y = k | X = x^{*})$ be for each $k = 1,2,3$, and why? This is known as the *zero-frequency* problem. 
