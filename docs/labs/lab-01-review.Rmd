...
subtitle: "due Sept. 19 at 11:59pm"
output: 
  tufte::tufte_html:
...
Sometimes, we want to obtain the elements of a vector or matrix which satisfy a certain condition. The function `which()` will return the indices of interest. For example, if I want to obtain the indices of the negative values in the following matrix `B`, I might do:
...
You will notice that the dataset is not loaded correctly becuase `R` has not differentiated the variable names from the rest of the data. The data set also includes a number of missing observations, indicated by a question mark.
...
2. Create a 2 x 2 matrix `B` that contains the values 1-4, filled row-wise. Then create another 2 x 2 matrix `C` that contains the values 5-8, filled column-wise. Perform the operations `B * C` and `B %*% C`. Comment on what each operation does.



Exercises 2-9 will continue working with the `auto` dataset. Make sure that the missing values have been removed from the data.
...
3. How many observations are in the cleaned data set?

4. Which of variables are quantitative, and which are qualitative?

5. What is the mean and standard deviation of each quantiative variable?

6. Obtain the correlations of the numeric variables. This can be achieved using the `cor()` function, where the input is a data frame or matrix of numeric values. Which two variables exhibit the most correlation? Which two variable exhibit are least correlated?


7. Suppose we are interested in predicting gas milage (`mpg`). Make a histogram of `mpg`. Don't forget to add a title!
