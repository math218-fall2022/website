...
- Go to the course organization on GitHub: [http://github.com/sta199-fa19](http://github.com/sta199-fa19). 
...
- Click [here](http://www2.stat.duke.edu/courses/Fall19/sta199.001/labs/data/evals-mod.csv) to download the data. The file is called `evals-mod.csv`.
- Navigate to the data folder in your project and upload the `evals-mod.csv` file.
- The codebook is available in the data section of [Lab 06](https://www2.stat.duke.edu/courses/Fall19/sta199.001/labs/lab-06-modeling-course-evals.html). 
...
2. Fit a linear model (one you have fit before) predicting average
   professor evaluation `score` based on average beauty rating (`bty_avg`) and `sex`. 
...
4. What percent of the variability in `score` is explained by the model in Exercise 3?

5.  What is the equation of the line corresponding to *just* male professors?
    
6.  For two professors who received the same beauty rating, which sex tends 
...
8. How do the adjusted $R^2$ values of the models from Exercises 2 and 3 compare? What does this tell us 
   about how useful `sex` is in explaining the variability in evaluation scores when we 
   already have information on the beaty score of the professor.

9. Compare the slopes of `bty_avg` under the two models. Has the 
   addition of `sex` to the model changed the parameter estimate (slope) for `bty_avg`?
    
10. Create a new model called `m_bty_rank` using `rank` and `bty_avg` to predict `score`. Write the equation of the linear model and interpret the slopes and intercept in context of the data. 

## Part 3: The search for the best model

Going forward, only consider the following variables as potential predictors: `rank`, 
`ethnicity`, `sex`, `language`, `age`, `cls_perc_eval`, `cls_did_eval`, `cls_students`, 
...
15. Using backward-selection with $AIC$ as the selection 
    ...
    write out the equation of linear model for predicting score based on your results. 
    
    Include `results = "hide"` in the code chunk header to suppress the output from the `step` function.
