...
Given these predictions, the `table()` function can be used to produce a confusion matrix in order to determine how many observations were correctly or incorrectly classified. It takes the predictions as the first argument, and the true values as the second argument.
...
set.seed(5)
n <- nrow(bdiag_dat)
train_ids <- sample(1:n, 0.5 * n) # randomly take 50% of indices as training
...
g) Repeat (e) using KNN with $K = 1$. First, make another dataset called `bdiag_dat_scale` which centers and scales the predictors.


h) Repeat (e) using KNN with $K = 15$
