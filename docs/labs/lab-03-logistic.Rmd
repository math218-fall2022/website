...
Here, we will work with the dataset `bdiag.csv`, which includes several imaging details from patients that had a biopsy to test for breast cancer. The variable `diagnosis` classifies the biopsied tissue as `M` = malignant or `B` = benign. In addition, ten real-valued features are computed for each cell:
...
bdiag_test <- bdiag_test[test_ids,] # create the test set
```

d) Fit a logistic regression model *on the training data* `bdiag_train` using the same predictors. Then obtain the confusion matrix and compute the misclassification rate *for the test data* `bdiag_test`.  

e) Repeat (d) using naive Bayes. 

f) Repeat (d) using KNN with $K = 1$.  First, make another dataset called `bdiag_scale` which centers and scales the predictors. Then use the same train and test IDs as before to create a `bdiag_scale_train` and `bdiag_scale_test` dataframe. Fit KNN to these scaled data.


g) Repeat (d) using KNN with $K = 15$ and your data from (f).
