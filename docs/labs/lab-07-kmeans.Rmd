...
subtitle: "Due Sunday, 11/17 at 11:59pm"
output: 
  tufte::tufte_html:
...
b) Using a `for()` loop, fit the $K$-means algorithm to this data for $K = 1, 2, \ldots, 10$. For each $K$, obtain and store the value of the objective. Don't forget to set a seed!
...
f) Based on your visualization in (e), try describing the types of customers within each cluster. Even though the features are standardized, we interpret positive values as being greater than average and negative values as lower. E.g. a standardized `Age` of 2 would correspond to a customer of old age.

## Exercise 3

Here, we will look at the same mall customer data, but this time perform $K$-means on the original, non-standardized data.

a) Create a new data frame called `mall` that only contains male customers, but does NOT standardize the features. 
...
b) Repeat steps (b)-(e) from Exercise 2. 
...
c) Discuss the differences that occurred when you ran $K$-means with standardized versus non-standardized data. You may want to discuss choosing an optimal $K$ and separation/interpretation of the resulting cluster assignments.
