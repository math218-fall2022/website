...
for (i in 1:10){
  cluster <- kmeans(mall_std[,3:5], centers=i, iter.max = 20, nstart=20, algorithm = "Lloyd")
...
```{r}
mall_std %>%
  mutate(cluster = factor(km_out_clusts)) %>%
...
tot.withinss <- rep(NA, 10)
for (i in 1:10){
  cluster <- kmeans(mall_dat[,3:5], centers=i, iter.max = 20, nstart=20, algorithm = "Lloyd")
  tot.withinss[i] <- cluster$tot.withinss
}
data.frame(K = 1:10, objective = tot.withinss) %>%
  ggplot(., aes(x = K, y = objective)) +
