...
a)  Split the data set into a 60% training set and a 40% test set, setting a seed for reproducibility. Use a seed of 10.
...
c) Fit a ridge regression model on the training set, with $\lambda$ chosen by cross-validation. Report the $\lambda$ and test error obtained. Use a seed of 10.
...
We will now fit a lasso model to that same simulated data. The following codes re-generates the `x` and `y` data for us:
